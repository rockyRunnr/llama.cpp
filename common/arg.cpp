...
common_params_context common_params_parser_init(common_params & params,
    ...
    add_opt(common_arg({ "--kv-paged" },
                       "[EXPERIMENTAL] enable paged KV cache with block-based management (default: disabled)",
                       [](common_params & params) { params.kv_paged = true; })
                .set_examples({ LLAMA_EXAMPLE_COMPLETION, LLAMA_EXAMPLE_CLI, LLAMA_EXAMPLE_SERVER,
                                LLAMA_EXAMPLE_PERPLEXITY, LLAMA_EXAMPLE_BENCH, LLAMA_EXAMPLE_PARALLEL }));
